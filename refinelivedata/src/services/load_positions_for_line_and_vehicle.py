...
# identifica as trips base apenas no sentido salvando start record e end record
# Para cada trip 
...
from src.infra.db import fetch_data_from_db_as_df, get_db_connection
import pandas as pd
...
def load_positions_for_line_and_vehicle(config, year, month, day, linha_lt, veiculo_id):
    table_name = config["POSITIONS_TABLE_NAME"]
    sql = f"""
        SELECT veiculo_ts, linha_sentido, distance_to_first_stop, lt_origem, lt_destino
        FROM {table_name}
        WHERE linha_lt = '{linha_lt}' AND veiculo_id = {veiculo_id}
        ORDER BY veiculo_ts ASC;
    """
    ...
    # --- LAYER 1: Group into Operational Legs (Sentido change or Time Gap) ---
    legs = []
    if records:
        current_leg = [records[0]]
        for i in range(1, len(records)):
            prev, curr = records[i - 1], records[i]

            # Split condition for the operational leg
            if curr["linha_sentido"] != prev["linha_sentido"]:
                # or (
                #     curr["veiculo_ts"] - prev["veiculo_ts"] > timedelta(hours=1)
                # ):
                legs.append(current_leg)
                current_leg = [curr]
            else:
                current_leg.append(curr)
        legs.append(current_leg)
