...
def test_freshness_pass():
    ...
    result = check_freshness(make_config(), df, now_fn=NOW_FN)
...
def test_freshness_empty_df_returns_fail_with_note():
    result = check_freshness(make_config(), pd.DataFrame(), now_fn=NOW_FN)
    assert result["status"] == "FAIL"
...
def test_freshness_result_contains_thresholds():
    df = make_df([SP_NOON - timedelta(minutes=5)])
    result = check_freshness(make_config(freshness_warn=10, freshness_fail=30), df, now_fn=NOW_FN)
...
def test_freshness_tz_aware_equivalent_to_tz_naive():
    ...
    result_naive = check_freshness(make_config(), df_naive, now_fn=NOW_FN)
    result_aware = check_freshness(make_config(), df_aware, now_fn=NOW_FN)
...
def test_recent_gaps_pass():
    ...
    result = check_recent_gaps(make_config(), df, now_fn=NOW_FN)
    assert result["status"] == "PASS"
...
def test_recent_gaps_empty_df_returns_fail_with_note():
    result = check_recent_gaps(make_config(), pd.DataFrame(), now_fn=NOW_FN)
    assert result["status"] == "FAIL"
...
def test_recent_gaps_result_contains_window():
    ...
    result = check_recent_gaps(make_config(gaps_window=60), df, now_fn=NOW_FN)
...
def test_recent_gaps_tz_naive_equivalent_to_tz_aware():
    ...
    result_aware = check_recent_gaps(make_config(), df_aware, now_fn=NOW_FN)
    result_naive = check_recent_gaps(make_config(), df_naive, now_fn=NOW_FN)
