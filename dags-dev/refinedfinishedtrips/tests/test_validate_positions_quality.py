...
def test_evaluate_freshness_below_warn_threshold():
    ...
    result = evaluate_freshness(make_config(), df, reference_datetime=NOW_UTC)
...
def test_evaluate_freshness_empty_df_returns_none_lag():
    result = evaluate_freshness(make_config(), pd.DataFrame(), reference_datetime=NOW_UTC)
    assert result["observed_lag_minutes"] is None


def test_evaluate_freshness_result_contains_thresholds():
    df = make_df([SP_NOON - timedelta(minutes=5)])
    result = evaluate_freshness(make_config(freshness_warn=10, freshness_fail=30), df, reference_datetime=NOW_UTC)
...
def test_evaluate_freshness_tz_aware_matches_tz_naive_lag():
    ...
    result_naive = evaluate_freshness(make_config(), df_naive, reference_datetime=NOW_UTC)
    result_aware = evaluate_freshness(make_config(), df_aware, reference_datetime=NOW_UTC)
...
def test_evaluate_recent_gaps_returns_max_gap():
    ...
    result = evaluate_recent_gaps(make_config(), df, reference_datetime=NOW_UTC)
...
def test_evaluate_recent_gaps_empty_df_returns_none_max_gap():
    result = evaluate_recent_gaps(make_config(), pd.DataFrame(), reference_datetime=NOW_UTC)
...
def test_evaluate_recent_gaps_result_contains_window():
    ...
    result = evaluate_recent_gaps(make_config(gaps_window=60), df, reference_datetime=NOW_UTC)
...
def test_evaluate_recent_gaps_tz_naive_matches_tz_aware_max_gap():
    ...
    result_aware = evaluate_recent_gaps(make_config(), df_aware, reference_datetime=NOW_UTC)
    result_naive = evaluate_recent_gaps(make_config(), df_naive, reference_datetime=NOW_UTC)
...
def test_validate_positions_quality_pass_when_both_checks_pass():
    ...
    result = validate_positions_quality(make_config(), df, reference_datetime=NOW_UTC)
...
def test_validate_positions_quality_uses_reference_datetime_instead_of_runtime_now():
    reference_datetime = NOW_UTC
    df = make_df([SP_NOON - timedelta(minutes=5)] * 2, extracao_ts_values=_ext_ts(27, 30))
    result = validate_positions_quality(
        make_config(),
        df,
        reference_datetime=reference_datetime,
    )
    freshness_check = next(check for check in result["checks"] if check["check"] == "freshness")
    assert freshness_check["status"] == "PASS"
    assert freshness_check["observed_lag_minutes"] == pytest.approx(5.0, abs=0.1)
