...
def load_trip_details_from_storage_to_dataframe(config):
    def get_config(config):
        try:
            ...
            return bucket_name, gtfs_folder, connection_data
        except KeyError as e:
            logger.error(f"Missing required configuration key: {e}")
            raise

    bucket_name, gtfs_folder, connection_data = get_config(config)
    # Path where you exported the data in the previous step
    s3_path = f"s3://{bucket_name}/{gtfs_folder}/trip_details/trip_details.parquet"
