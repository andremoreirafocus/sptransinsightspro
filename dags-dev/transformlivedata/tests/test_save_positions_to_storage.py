import pandas as pd
import pytest
from transformlivedata.services.save_positions_to_storage import save_positions_to_storage
...
def test_returns_early_when_df_is_empty():
    fake_con = FakeDuckDBConnection()
    save_positions_to_storage(make_config(), pd.DataFrame(), "trusted", duckdb_client=fake_con)
...
def test_trusted_bucket_resolved_from_config():
    ...
    save_positions_to_storage(make_config(), make_df(), "trusted", duckdb_client=fake_con)


def test_quarantined_bucket_resolved_from_config():
    fake_con = FakeDuckDBConnection()
    save_positions_to_storage(make_config(), make_df(), "quarantined", duckdb_client=fake_con)
