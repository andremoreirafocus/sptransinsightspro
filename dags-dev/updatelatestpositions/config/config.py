...
def get_config():
    ...
    if os.getenv("AIRFLOW_HOME"):
        ...
        return get_local_config(dotenv_values("updatelatestpositions/.env"))


def get_local_config(env_values):
    base_dir = os.path.dirname(os.path.abspath(__file__))
    config_path = os.path.join(base_dir, "updatelatestpositions.json")
    with open(config_path, "r") as f:
        general = json.load(f)["updatelatestpositions_general"]
...
def get_airflow_config():
    ...
    updatelatestpositions_vars = Variable.get(
        "updatelatestpositions_general", deserialize_json=True
    )
    ...
    storage = updatelatestpositions_vars["storage"]
    tables = updatelatestpositions_vars["tables"]
