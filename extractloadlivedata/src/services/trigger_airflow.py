...
def create_pending_invokation(
    ...
    structured_logger.info(
        event="airflow_invocation_cache_started",
    ...
    try:
        ...
        structured_logger.error(
            ...
            message=f"Failed to create pending invokation for file '{filename}': {e}",
            error_type=type(e).__name__,
            error_message=str(e),
        )
        raise
    structured_logger.info(
        event="airflow_invocation_cache_succeeded",
...
def remove_pending_invokation(
    ...
    try:
        ...
        structured_logger.error(
            event="pending_storage_file_failed",
            status=EVENT_STATUS_FAILED,
            message=f"Failed to remove pending invokation marker '{marker_name}': {e}",
            error_type=type(e).__name__,
            error_message=str(e),
        )
        raise
...
def get_pending_invokations(
    ...
    try:
        ...
        structured_logger.error(
            ...
            status=EVENT_STATUS_FAILED,
            message=f"Failed to retrieve pending invokations: {e}",
            error_type=type(e).__name__,
            error_message=str(e),
        )
        raise
    structured_logger.debug(
        event="airflow_invocation_scan_succeeded",
        status=EVENT_STATUS_SUCCEEDED,
        message=f"Found {len(pending_markers)} pending invokation(s).",
        metadata={"pending_count": len(pending_markers)},
    )
...
def trigger_pending_airflow_dag_invokations(
    ...
    if pending_markers:
        for pending_marker in pending_markers:
            structured_logger.debug(
                event="airflow_invocation_detected",
                status=EVENT_STATUS_STARTED,
                message=f"Processing pending invokation '{pending_marker}'.",
                metadata={"pending_count": len(pending_markers)},
            )
            try:
...
def get_utc_logical_date_from_file(pending_marker: str) -> str:
    try:
        ...
        structured_logger.error(
            ...
            status=EVENT_STATUS_FAILED,
            message=f"Failed to extract logical date from marker '{pending_marker}': {e}",
            error_type=type(e).__name__,
            error_message=str(e),
        )
        raise
...
def trigger_airflow_dag_run(
    ...
    try:
        ...
        structured_logger.error(
            ...
            status=EVENT_STATUS_FAILED,
            message=f"Failed to read Airflow configuration: {e}",
            error_type=type(e).__name__,
            error_message=str(e),
        )
        raise IngestNotificationError("failed to read Airflow configuration") from e
    ...
    structured_logger.info(
        ...
        status=EVENT_STATUS_STARTED,
        message=f"Triggering Airflow DAG for marker '{pending_marker}'.",
        metadata={
