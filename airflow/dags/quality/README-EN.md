...
Builds the standardized summary block consumed by the alertservice.
...
Pass a caller-assembled `details` dict to preserve partial pipeline results collected before the failure, for example transformation metrics captured before a late-stage GX exception.
