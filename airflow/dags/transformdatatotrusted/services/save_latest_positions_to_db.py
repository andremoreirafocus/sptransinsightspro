from transformdatatotrusted.infra.db import bulk_insert_data_table, execute_sql_command
import logging
...
def save_latest_positions_to_db(config, latest_positions_table):
    """
    ...
    truncate_sql = f"TRUNCATE TABLE {latest_positions_table_name};"
    execute_sql_command(config, truncate_sql)

    insert_sql = f"""
    INSERT INTO {latest_positions_table_name} (
    ...
    bulk_insert_data_table(config, insert_sql, latest_positions_table)
