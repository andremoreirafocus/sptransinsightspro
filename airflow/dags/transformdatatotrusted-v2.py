...
from transformdatatotrusted.services.save_latest_positions_to_db import (
    save_latest_positions_to_db,
)
...
def save_latest_to_db(ti):
    config = get_config()
    positions_table = ti.xcom_pull(task_ids="transform_positions_to_in_memory_table")
    save_latest_positions_to_db(config, positions_table)


# Criando o DAG
with DAG(
    "transformdatatotrusted-v2",
    ...
    save_latest_to_db_task = PythonOperator(
        task_id="save_latest_to_db",
        python_callable=save_latest_to_db,
    )

    (
        load_from_raw_task
        >> transform_positions_task
        >> save_to_db_trusted_task
        >> save_latest_to_db_task
    )
