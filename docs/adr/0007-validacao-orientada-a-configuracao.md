# ADR-0007: Validação orientada a configuração (JSON Schema + Great Expectations)
...
O [ADR-0004](./0004-framework-de-qualidade-de-dados.md) estabelece que o `transformlivedata` deve tratar qualidade de dados como cidadã de primeira classe, com validação em múltiplos estágios: JSON Schema antes da transformação e Great Expectations após. Esta decisão pressupõe esse framework já adotado e trata de uma questão operacional específica: **onde as regras de validação devem ser armazenadas**.

A abordagem mais simples — inserir regras "hardcoded" no código Python (`if "field" not in data: raise`, thresholds numéricos fixos) — funciona para regras estáticas, mas torna impossível atualizar as regras sem redeployment do pipeline, e mistura a lógica de validação com a lógica de negócio. Em produção com Airflow, ajustar um único threshold exigiria Pull Request + merge + promoção.
...
**Validação em Python hardcoded:** Simples e sem dependências. Porém, qualquer mudança nas regras exige modificação de código, lint, testes e redeployment. Em produção com Airflow, isso significa uma Pull Request + merge + promoção, mesmo para ajustar um threshold numérico.
...
- Possibilidade de analistas de dados ou outras pessoas do time de dados adicionrem, removerem ou alterarem as regras de validação sem necessidade da participação do engenheiro de dados.
